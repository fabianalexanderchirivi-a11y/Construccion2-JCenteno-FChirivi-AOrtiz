...
class OrderDiagnosticAidSingleItemTest {

    @Test
    void diagnosticAidOrderAllowsOnlyThatSingleItem() {
        Order order = makeOrder("ORD-DA-1", "2001", "D-11", LocalDateTime.now());
        order.addItem(makeDiagnosticAid("ORD-DA-1", 1, "IMG-001", "RX mano", new BigDecimal("60000")));
        assertThrows(IllegalStateException.class, () ->
                order.addItem(makeDiagnosticAid("ORD-DA-1", 2, "IMG-002", "RX pie", new BigDecimal("60000")))
        );
        assertThrows(IllegalStateException.class, () ->
                order.addItem(makeProcedure("ORD-DA-1", 2, "PROC-005", "Curación", new BigDecimal("15000"), 1))
        );
        assertThrows(IllegalStateException.class, () ->
                order.addItem(makeMedication("ORD-DA-1", 2, "MED-005", "Diclofenaco", new BigDecimal("800"), 3, "50 mg c/8h"))
        );
    }
